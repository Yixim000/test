...
int main()
{
 ...
 circlevec.push_back(*shape);
    ...
    ss << score;
 point.setText(sc + ss.str().c_str(), "arial.ttf");
 point.setSprite("texture.png");

    bool hearts{}; //this should work but it does not
    while (window.isOpen())
    {
        sf::Event event;
        ...
        if (isSpriteHover(one.sprite.getGlobalBounds(), sf::Vector2f(event.mouseButton.x, event.mouseButton.y)) == true)
        {
            ...
            while (window.isOpen()) {
                while (window.pollEvent(event)) {
                 ...
                 if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {//nie działą z jakiegoś powodu od kiedy jest z num
                        ...
                        for (int i = 0; i < 2; i++) {
                            ...
                            if (shapeVec[i].selected)
                            {
                                ...
                                if ((isSpriteHover(four.sprite.getGlobalBounds(), sf::Vector2f(shapeVec[i].shape.getPosition().x, shapeVec[i].shape.getPosition().y)) == true))
                                {
                                        ...
                                        if (i == 0) // okej znazłam swój błąd ta pętla się mega dużo razy odbywa nie dwa
                                        {
